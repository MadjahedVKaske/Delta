...
Процедура Buro1Goz_ПриСозданииНаСервереПосле(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Об.ЗаявкаНаРасходованиеДенежныхСредств) Тогда
		
		Если ЗначениеЗаполнено(Об.ЗаявкаНаРасходованиеДенежныхСредств.ГозКонтракт) Тогда
			
			Объект.ГозКонтракт = Об.ЗаявкаНаРасходованиеДенежныхСредств.ГозКонтракт;	
			
		КонецЕсли;
		
	КонецЕсли;
	
	ПолеПроектРазвития = Элементы.Добавить("ПроектРазвития", Тип("ПолеФормы"), Элементы.ГруппаДополнительныеРеквизитыШапки);
