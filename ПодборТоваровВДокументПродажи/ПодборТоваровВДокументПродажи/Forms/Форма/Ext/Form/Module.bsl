...
Процедура ПриОткрытии(Отказ)
	...
	// Заполняем остатки при открытии формы
	ЗаполнитьОстаткиВКорзине(Организация);
...
Процедура КорзинаНоменклатураПриИзменении(Элемент)
	...
	// Обновляем остатки при изменении номенклатуры
	ЗаполнитьОстаткиВКорзине(Организация);
...
Функция ДобавитьВКорзинуНаКлиенте(ПараметрыТовара, НовыеСтроки)
	...
	// Заполняем остатки для добавленных товаров
	ЗаполнитьОстаткиВКорзине(Организация);
...
// Получить остатки товаров по организации
//
// Параметры:
//  Организация - СправочникСсылка.Организации - Организация для получения остатков
//
&НаСервере
Процедура ЗаполнитьОстаткиВКорзине(Организация)
	
	Если Не ЗначениеЗаполнено(Организация) Тогда
		Возврат;
	КонецЕсли;
	
	// Собираем уникальные комбинации Номенклатура+Характеристика
	...
	|ИЗ
	|	РегистрНакопления.ТоварыНаСкладах.Остатки(, Организация = &Организация И (Номенклатура, Характеристика) В (ВЫБРАТЬ Номенклатура, Характеристика ИЗ ВТ_Номенклатура)) КАК ТоварыНаСкладахОстатки
	|
	|СГРУППИРОВАТЬ ПО
	...
	Пока Выборка.Следующий() Цикл
		Ключ = Строка(Выборка.Номенклатура) + "|" + Строка(Выборка.Характеристика);
		СоответствиеОстатков.Вставить(Ключ, Выборка.Остаток);
	КонецЦикла;
	
	// Заполняем остатки в корзине
	Для Каждого Строка Из Объект.Корзина Цикл
		Если ЗначениеЗаполнено(Строка.Номенклатура) Тогда
			Ключ = Строка(Строка.Номенклатура) + "|" + Строка(Строка.Характеристика);
			Остаток = СоответствиеОстатков.Получить(Ключ);
